  ...
  UINT64 EfsSizeAddr)
{
  ...
  //
  // 允许 Windows/Linux 和调制解调器子系统访问共享内存区域。
  // 这是必需的，否则调制解调器子系统在尝试读取数据时会崩溃。
  //
...
{
  EFI_MEMORY_REGION_DESCRIPTOR MpssEfs;

  if (!EFI_ERROR (LocateMemoryRegionByName ("MPSS_EFS", &MpssEfs))) {
